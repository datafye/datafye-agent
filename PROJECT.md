...
Each commentary line is both emitted live as a `commentary` SSE event *and* appended to the conversation's on-disk commentary log, so the activity panel can be replayed when a user reopens a project (`GET /v1/conversations/{id}/history` returns both `messages` and `commentary`). We used to cap that log at 400 entries (`_MAX_COMMENTARY`); we removed the cap. The commentary trail is the *analytics record* accounts persists — a long session's early activity is exactly the part you don't want silently dropped — so it now grows unbounded.

### Narration routing: work goes to the Work panel, the answer goes to the Conversation

The agent narrates as it works ("Provisioning the SIP dataset…", "Running the backtest…") and then gives a final answer. Left alone, all of that text lands in the Conversation as one run-on wall — the play-by-play tangled up with the actual reply. We fixed this **entirely server-side, with no frontend change**, by watching the *shape* of the stream.

The insight: a text burst that is **followed by a tool call** is almost always work-narration — Claude saying what it's about to do — whereas the **final trailing burst**, the one with no tool call after it, is the actual reply. So `/v1/chat` buffers each burst in `pending_burst`; when a tool call arrives, it flushes the buffered burst to the Work panel as a `"muted"` commentary line (`append_commentary(.., "muted")`) rather than the Conversation. Whatever burst is still buffered when the turn ends (the `ResultMessage` branch) is the reply: it's emitted as `content`, persisted as the assistant message (`conversation_text`), and that's the only text the Conversation shows. `prompt.py` was reshaped to cooperate — short action lines while working, a plain high-level final message, and no "Let me…" openers. The lesson: you can often route a stream by its *structure* (what follows what) without the model having to tag each chunk, and without teaching the frontend a new event.
