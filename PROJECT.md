...
The insight: a text burst that is **followed by a tool call** is almost always work-narration — Claude saying what it's about to do — whereas the **final trailing burst**, the one with no tool call after it, is the actual reply. So `/v1/chat` buffers unrouted text as a **list of distinct blocks** (`pending_blocks`) — deliberately *not* a single concatenated string. When a tool call arrives, each buffered block is flushed to the Work panel as **its own** `"muted"` commentary line (`append_commentary(.., "muted")`), so a multi-sentence narration shows up as one Work line per sentence instead of a run-on blob with the sentences jammed together at the periods (the bug we got from string concatenation). Whatever blocks are still buffered when the turn ends (the `ResultMessage` branch) are the reply: they're joined with `\n\n` into `conversation_text`, emitted as `content`, persisted as the assistant message, and that's the only text the Conversation shows. `prompt.py` was reshaped to cooperate — short action lines while working, a plain high-level final message, and no "Let me…" openers. The lesson: you can often route a stream by its *structure* (what follows what) without the model having to tag each chunk, and without teaching the frontend a new event.
