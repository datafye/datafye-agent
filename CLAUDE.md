...
- **Narration routing (no frontend change)**: the streamer buffers each text burst; a burst *followed by a tool call* is work-narration and goes to the Work panel (`append_commentary(.., "muted")`), while the final trailing burst is the reply and goes to the Conversation (via `pending_burst`/`conversation_text`, flushed + persisted in the ResultMessage branch). `prompt.py` was reshaped to emit short action lines (Work panel) + a plain high-level final message (Conversation), no "Let me…" openers
