...
| `commentary` | Background-activity line for the workspace activity panel (`{text}`). Emitted for notable tool calls — Bash and MCP — and for **work-narration** (a text burst that is followed by a tool call, routed here at `"muted"` level instead of to the Conversation). Also appended to the conversation's commentary audit trail (now **uncapped** — the cap was removed since commentary is the analytics record accounts persists) |
...
- **Narration routing (no frontend change)**: the streamer buffers unrouted text as a **list of distinct blocks** (`pending_blocks`, one per narration sentence / reply paragraph — NOT a concatenated string, so sentences never glue into a run-on line at their periods). Blocks *followed by a tool call* are work-narration: each is flushed to the Work panel as **its own commentary line** (`append_commentary(.., "muted")`, one Work line per sentence), while the final trailing blocks are the reply and go to the Conversation (joined with `\n\n` into `conversation_text`, flushed + persisted in the ResultMessage branch). `prompt.py` was reshaped to emit short action lines (Work panel) + a plain high-level final message (Conversation), no "Let me…" openers
